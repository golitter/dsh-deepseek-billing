...
- `test/index.test.js`：15 个宿主端用例，覆盖凭据处理、五个固定错误码、缺失/未知 `.code` 兜底、超时与网络/HTTP 错误、非法响应、字段白名单、空余额、GET/405 路由、`/deepseek-billing` 命令（zh/en 菜单说明与结果本地化、语言更新重注册、设置读取失败和语言中性回退）及配置边界。
