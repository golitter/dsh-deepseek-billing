...
- **访问控制边界**：DSH WebServer 当前允许绑定 `127.0.0.1` 或 `0.0.0.0`。非 loopback 部署需要由 DSH connection 层配置受信 authority；该信任列表只防御 DNS rebinding，不提供身份认证。余额路由携带凭据能力并返回账户数据，因此在路由入口复刻 DSH 自己的 `/api` browser-trust fence，并以**空信任列表独立锁定 loopback**（拒绝 cross-site、拒绝跨域 Origin），未通过返回 `403 { ok:false, code }`。通过 LAN 或远程地址打开 Web UI 时，普通页面按部署配置工作，但“计费”页的余额请求返回 `403`，这是有意的安全限制；`--trusted-host` 是 DNS-rebinding 白名单而非鉴权，故不放开余额。若未来 DSH 提供官方鉴权/会话边界，应改为复用而非自建密钥。
