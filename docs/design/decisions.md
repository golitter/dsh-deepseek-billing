...
11. **复刻 DSH 的 browser-trust fence 并锁定 loopback**：DSH WebServer 可以绑定 `127.0.0.1` 或 `0.0.0.0`，非 loopback 部署由 DSH connection 层配置受信 authority；该列表只防御 DNS rebinding，不提供身份认证。`exact` 路由在 webserver 匹配中优先于 `/api` 前缀路由，会绕过连接层自带的 fence，因此路由在入口自行复刻同一道「Host loopback + 同源」判定并返回 `403 { ok:false, code }`。余额读取 API Key、发起上游、返回账户数据，属于高权限操作，故用空信任列表独立锁定 loopback，与 DSH `credentials`/`settings` 平面一致，`--trusted-host`（DNS-rebinding 白名单，非鉴权）不放开它；LAN/远程 Web UI 的普通页面可按部署配置工作，但余额请求有意返回 `403`。
...
13. **客户端请求超时对齐宿主 `timeoutMs`**：宿主超时只覆盖上游段，本地连接挂起会让余额页永远停留在加载态、刷新按钮持续禁用。客户端为每个请求设置 `AbortController` 定时器，初始值为宿主上游超时上限（`HOST_MAX_TIMEOUT_MS` 镜像 `lib/index.js` 的 `MAX_TIMEOUT_MS` = 120s，跨文件契约，需同步修改）+ 5s 余量，保证首轮请求永不早于宿主超时；此后从每个通过 fence 的响应 envelope 附带的 `timeoutMs` 学习实际配置，对齐为 `timeoutMs + 5s` 并钳制在默认上限内（畸形值忽略，fence 前 `403` 不携带）。自身超时触发的 abort 呈现为稳定的 `balance_timeout` 错误态，被取代/卸载的 abort 仍静默。
