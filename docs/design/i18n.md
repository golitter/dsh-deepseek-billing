...
| `refreshError.message` | 刷新失败：{reason} | Refresh failed: {reason} |
...
约束：`zh`、`en` 键必须完全一致（21 键）；新增错误码时同步更新 `ERROR_KEY`、两份词典和 README。刷新失败提示复用错误码翻译，并通过 `refreshError.message` 插值生成，不直接拼接可见文案。
...
`currency` 来自经过客户端响应校验的 `balance.currency`，缺省 `'CNY'`，原样透传。切语言只改文字，绝不改金额或币种。
