  ...
  factory: (require) => {
    ...
    `
      ...
      ctx.on('command/executed', (sessionId, commandName, result) => {
        ...
        const notice = input.notices?.getSnapshot()
        if (notice !== null && notice !== undefined && typeof input.notices?.set === 'function') {
          const owned = { store: input.notices, notice, timer: undefined, unsubscribe: () => {} }
      ...
      ctx.slots.inject('settings.section', () => ctx.slots.register(
        { name: 'settings.section', id: 'billing', order: 100, label: () => t('nav') },
        BillingSection,
      ))
    }
