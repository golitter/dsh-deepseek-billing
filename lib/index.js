 ...
 * authenticated browser cookie. The route is registered through DSH
 * Connection, which owns the browser trust and authentication boundary.
 */
...
export const Config = z.object({
  ...
  allowCustomEndpoint: z.boolean().default(false),
  timeoutMs: z.number().min(1).max(MAX_TIMEOUT_MS).default(DEFAULT_TIMEOUT_MS),
  maxRequestsPerMinute: z.number().step(1).min(1).default(DEFAULT_MAX_REQUESTS_PER_MINUTE),
})
...
function rateLimitKey(request) {
  ...
  if (typeof cookie !== 'string' || cookie.length === 0) return 'authenticated-fallback'
...
export function apply(ctx, config = {}) {
  ...
  ctx.effect(() => ctx.connection.fetch.register({
    ...
    fetch: async (request) => {
      const respond = (status, payload, headers = {}) => new Response(JSON.stringify(payload), {
        status,
        headers: {
      ...
      // Plugin envelopes (success, rejection, failure) carry the configured
      // upstream timeout so the Client half can align its own request timer
      // to `timeoutMs + margin` instead of guessing. Connection-generated
      // authentication responses never enter this handler.
      const respondTrusted = (status, payload, headers = {}) => {
        return respond(status, { ...payload, timeoutMs }, headers)
      }

      if (request.method !== 'GET') return respondTrusted(405, { ok: false, code: ERROR_CODES.BILLING_SERVICE_UNAVAILABLE }, { allow: 'GET' })

      const clientKey = rateLimitKey(request)
      if (!rateLimiter(clientKey)) {
        return respondTrusted(429, { ok: false, code: ERROR_CODES.BILLING_SERVICE_UNAVAILABLE })
      }

      try {
        const balance = await service.getBalance()
        return respondTrusted(200, { ok: true, balance })
      } catch (error) {
        ...
        return respondTrusted(502, { ok: false, code })
      }
    },
