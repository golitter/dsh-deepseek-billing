 ...
 * client. The WebServer may be deployed on a non-loopback authority, but this
 * privileged balance route remains loopback-only. It re-applies DSH's own
 * `/api` browser-trust fence (Host loopback + same-origin), which an `exact`
 * route would otherwise bypass; the fence is an authorization boundary for
 * this route, not a statement about the WebServer's bind capability.
 */
...
// Cross-file contract: lib/client.js's HOST_MAX_TIMEOUT_MS (its pre-learning
// request bound) mirrors this cap. If this is ever raised, update the client
// constant in the same change or a slow-but-configured upstream can be cut
// short by the client's default timer.
 ...
 * header must name loopback, cross-site fetch metadata is refused, and any
 * Origin must be same-origin. This is the DNS-rebinding / cross-site defense
 * for a local GUI — not an authentication layer. The WebServer may bind
 * `0.0.0.0`, but this privileged route is pinned to loopback and, unlike
 * DSH's own fence, deliberately has no trusted-authority widening (so a
 * `--trusted-host` entry never reaches the key-bearing upstream call).
 */
 ...
 * client addresses, so loopback clients (the only supported balance
 * deployment) share one budget. Stale windows are pruned on every check to
...
export function apply(ctx, config = {}) {
  ...
  if (!Number.isFinite(timeoutMs) || timeoutMs <= 0 || timeoutMs > MAX_TIMEOUT_MS) {
    throw new TypeError(`deepseek-billing: timeoutMs must be greater than 0 and no more than ${MAX_TIMEOUT_MS}ms`)
  }
  ...
  }, 'deepseek-billing: active balance requests')
  ...
  ctx.effect(() => ctx.webServer.register({
    ...
    handler: async (req, res) => {
      ...
      // Post-fence envelopes (success, rejection, failure) carry the configured
      // upstream timeout so the Client half can align its own request timer
      // to `timeoutMs + margin` instead of guessing. Pre-fence 403s go to
      // untrusted requesters and deliberately omit it.
      const respondTrusted = (status, payload, headers = {}) => {
        respond(status, { ...payload, timeoutMs }, headers)
      }
      ...
      // account data — so it is pinned to loopback with no trusted-authority
      // widening, the same way DSH gates its credentials/settings plane.
      // `--trusted-host` is a DNS-rebinding fence, not authentication, and
      // must not widen this.
      ...
      if (req.method !== 'GET') {
        respondTrusted(405, { ok: false, code: ERROR_CODES.BILLING_SERVICE_UNAVAILABLE }, { allow: 'GET' })
        return
      }

      const clientKey = req.socket?.remoteAddress ?? 'unknown'
      if (!rateLimiter(clientKey)) {
        respondTrusted(429, { ok: false, code: ERROR_CODES.BILLING_SERVICE_UNAVAILABLE })
        return
      }

      try {
        const balance = await service.getBalance()
        respondTrusted(200, { ok: true, balance })
      } catch (error) {
        ...
        respondTrusted(502, { ok: false, code })
      }
    },
